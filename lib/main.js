...
(function(root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        define(['es6-polyfills/lib/promise', 'es6-polyfills/lib/object', 'stream', 'console'], factory);
    } else if (typeof module === 'object' && module.exports) {
        module.exports = factory(require('es6-polyfills/lib/promise'), require('es6-polyfills/lib/object'), require('stream'), require('console'));
    }
...
function factory(Promise, Object, stream, console)
{
	...
	if (!stream.Writable.prototype.isPrototypeOf(options.stdout) && !stream.Duplex.prototype.isPrototypeOf(options.stdout)) {
	    throw new Error('stdout is not a prototype of stream.Writable');
	}
	if (!stream.Writable.prototype.isPrototypeOf(options.stderr) && !stream.Duplex.prototype.isPrototypeOf(options.stderr)) {
	    throw new Error('stderr is not a prototype of stream.Writable');
	}
